(function(){
  'use strict';
  if(window.__trainerV13FixesInstalled) return;
  window.__trainerV13FixesInstalled=true;
  ...
  function sanitizeContext(item,maps){
    ...
    return Object.assign({},item,{
      q:'Вставь '+label+' в предложение: '+clean(src.cloze),
      e:done ? 'Правильное предложение: '+done+(tr?' — '+tr:'') : ''
    });
  }
  ...
  try{
    if(typeof filteredExercises==='function'){
      ...
      filteredExercises=function(){
        ...
        let items;
        selectedMode='all';
        try{ items=baseFiltered(); }
        finally{ selectedMode=mode; }
  ...
  try{
    if(!window.__trainerRefreshStateInstalled){
      ...
      let refreshRestoreDone=false;
      function restoreRefreshPosition(){
        if(refreshRestoreDone) return;
        refreshRestoreDone=true;
        const restored=restoreTrainerRefreshState();
        refreshStateReady=true;
        if(!restored) saveTrainerRefreshState();
      }
      function queueRefreshStateSave(){
        setTimeout(function(){ if(refreshStateReady) saveTrainerRefreshState(); },0);
      }
      document.addEventListener('click',queueRefreshStateSave);
      document.addEventListener('change',queueRefreshStateSave);
      ...
      if(document.readyState==='complete') setTimeout(restoreRefreshPosition,50);
      else window.addEventListener('load',function(){setTimeout(restoreRefreshPosition,50);},{once:true});
      setInterval(function(){if(refreshStateReady && document.visibilityState==='visible') saveTrainerRefreshState();},5000);
    }
  }catch(e){}
})();
