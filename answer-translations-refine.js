(function(){
  ...
  if(window.__answerTranslationsRefineInstalled) return;
  window.__answerTranslationsRefineInstalled=true;
  ...
  if(typeof window.__answerTranslationsBasePair!=='function' &&
     typeof window.answerPairForExercise==='function'){
    window.__answerTranslationsBasePair=window.answerPairForExercise;
  }

  function basePair(item){
    const fn=window.__answerTranslationsBasePair;
    if(typeof fn==='function') return fn(item);
    const current=window.answerPairForExercise;
    if(typeof current==='function'&&current!==refinedPair) return current(item);
    return {es:'',ru:''};
  }

  function refinedPair(item){
    let pair=basePair(item);
  ...
  try{
    if(typeof render==='function'&&!render.__answerTranslationsRefinedV2){
      const base=render;
      const patched=function(){
        base();
        try{paint();}catch(e){}
      };
      patched.__answerTranslationsRefinedV2=true;
      patched.__answerTranslationsV2=!!base.__answerTranslationsV2;
      render=patched;
    }
  }catch(e){}

  if(!document.getElementById('answer-translations-refine-style')){
    const outfitStyle=document.createElement('style');
    outfitStyle.id='answer-translations-refine-style';
