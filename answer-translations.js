(function(){
  ...
  if(window.__answerTranslationsInstalled) return;
  window.__answerTranslationsInstalled=true;
  ...
  function install(){
    try{
      if(typeof render==='function'&&!render.__answerTranslationsV2){
        const base=render;
        const patched=function(){base();try{renderAnswerTranslation();}catch(e){}};
        patched.__answerTranslationsV2=true;
        render=patched;
      }
  ...
  if(!document.getElementById('answer-translations-style')){
    const style=document.createElement('style');
    style.id='answer-translations-style';
