(function(){
  const ASSET_VERSION="20260918-r2";
  const withVersion=(url)=>url+(url.includes("?")?"&":"?")+"v="+encodeURIComponent(ASSET_VERSION);
  ...
  function spriteFrom(imageKey,mapKey,key,cols,rows,hidden){
    const pos=(MAP[mapKey]||{})[key];
    const src=IMG[imageKey];
    if(!pos||!src) return "";
    const left=-(pos[0]*100);
    const top=-(pos[1]*100);
    return '<div class="generated-sprite generated-sprite-'+mapKey+'"'+(hidden?' hidden':'')+' aria-hidden="true">'+
      '<img class="generated-sprite-img" src="'+src+'" alt="" aria-hidden="true" loading="eager" decoding="async" '+
      'style="width:'+(cols*100)+'%;height:'+(rows*100)+'%;left:'+left+'%;top:'+top+'%" '+
      'onerror="this.hidden=true;this.parentElement.classList.add(\'asset-missing\')">'+
      '</div>';
  }
  ...
  function firstSprite(options){
    ...
    return '<div class="generated-sprite asset-missing" aria-hidden="true"></div>';
  }
  ...
  window.clothingArt=function(kind){
    const directRaster={
      hoodie:withVersion("assets/picture-labels/clothes-new-raster/hoodie.webp"),
      vest:withVersion("assets/picture-labels/clothes-new-raster/vest.webp"),
      polo:withVersion("assets/picture-labels/clothes-new-raster/polo.webp"),
      cardigan:withVersion("assets/picture-labels/clothes-new-raster/cardigan.webp")
    };

    if(directRaster[kind]){
      const fallback=spriteFrom("clothesNew","clothesNew",kind,2,2,true);
      return '<div class="new-clothes-art">'+
        '<img class="precise-art new-clothes-raster" src="'+directRaster[kind]+'" alt="" aria-hidden="true" loading="eager" decoding="async" '+
        'onerror="this.hidden=true;var f=this.nextElementSibling;if(f){f.hidden=false}else{this.parentElement.classList.add(\'asset-missing\')}">'+
        fallback+
        '</div>';
    }
  ...
  generatedSpriteStyle.textContent=
    '.food-art .generated-sprite,.new-clothes-art{width:min(100%,360px);aspect-ratio:1/1;flex:0 0 auto}'+
    '.generated-sprite{position:relative;overflow:hidden;background:#fff}'+
    '.generated-sprite[hidden]{display:none!important}'+
    '.generated-sprite-img{position:absolute;display:block;max-width:none!important;object-fit:fill}'+
    '.new-clothes-art{position:relative;display:grid;place-items:center;overflow:hidden}'+
    '.new-clothes-art>.precise-art{width:100%;height:100%;object-fit:contain;display:block}'+
    '.new-clothes-art>.generated-sprite{width:100%;height:100%}'+
    '.asset-missing{display:grid!important;place-items:center;background:#f7f5fc;border:1px dashed #d8d3e8}'+
    '.asset-missing::after{content:"🖼️";font-size:32px;opacity:.45}'+
    '@media(max-width:520px){.food-art .generated-sprite,.new-clothes-art{width:min(100%,260px)}}';
