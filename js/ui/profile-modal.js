...
#btn-profile { background:transparent; border:none; outline:none; box-shadow:none; cursor:pointer; font-size:1.3rem; padding:4px 8px; line-height:1; border-radius:12px; display:inline-flex; align-items:center; justify-content:center; transition:background .2s ease, transform .1s ease; }
...
`;
  ...
  function initProfileModal() {
    ...
    profileBtn.setAttribute('style','background:transparent; border:none; cursor:pointer; font-size:1.3rem; padding:4px 8px; line-height:1;');
    ...
    modal.innerHTML=`<button class="close-btn" type="button">×</button><h2>Мой профиль</h2><div class="profile-key-box"><span class="profile-key-value" id="profileCodeValue"></span><button class="btn-primary-action" id="copyProfileCode" type="button">Копировать</button></div><p>Сохрани этот ключ. Введи его на другом устройстве, чтобы продолжить обучение со всеми трофеями и прогрессом.</p><div class="sync-status-row"><span class="sync-dot"></span><span>Синхронизировано только что</span></div><div class="restore-section"><h3>Вход на новом устройстве</h3><div class="restore-input-group"><input id="profileCodeInput" placeholder="Введите GATO-XXXX-XXXX"><button class="btn-primary-action" id="restoreProfileBtn" type="button">Восстановить</button></div></div>`;
