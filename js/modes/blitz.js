(function(){
  ...
  let chooser,game,banner,session,current,phaseTimer=0,transitionTimer=0,lastCount=10,lastTopicId="all";
  ...
  function start(count,pool,topicId){
    ...
    session={source,dictionary:dictionary.length?dictionary:source,topicId,total:queue.length,queue,completed:new Set(),attempts:0,retries:0,errors:0,startedAt:Date.now()};
    game.hidden=false;
    lock(true);
    syncBanner();
    window.dispatchEvent(new CustomEvent("photoflash:start",{detail:{count:queue.length,topicId}}));
    next();
    return true;
  }
  ...
  function ui(){banner=document.getElementById("photoflashBanner");chooser=document.createElement("dialog");chooser.className="pf-dialog";chooser.innerHTML='<div><header><div><b>⚡ Новый режим</b><h2>Фотовспышка</h2><p>Выбери тему и размер быстрого спринта.</p></div><button class="pf-close" data-pf-dialog-close aria-label="Закрыть">×</button></header><label class="pf-topic-field"><span>Тема</span><select data-pf-topic><option value="all">Все темы (микс)</option></select></label><div class="pf-counts">'+COUNTS.map(n=>'<button type="button" data-pf-count="'+n+'"><b>'+n+'</b><span>слов</span></button>').join("")+'</div><input type="hidden" data-pf-selected value="10"><div class="pf-actions"><button class="pf-primary" data-pf-go>Поехали!</button></div></div>';document.body.appendChild(chooser);game=document.createElement("section");game.className="pf-game";game.hidden=true;document.body.appendChild(game);}
  function bind(){if(banner){banner.addEventListener("click",openChooser);banner.addEventListener("keydown",e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();openChooser();}});}chooser.addEventListener("click",e=>{const c=e.target.closest("[data-pf-count]");if(c){chooser.querySelector("[data-pf-selected]").value=c.dataset.pfCount;chooser.querySelectorAll("[data-pf-count]").forEach(b=>b.classList.toggle("active",b===c));return;}if(e.target.closest("[data-pf-dialog-close]"))return chooser.close();if(e.target.closest("[data-pf-go]"))start(Number(chooser.querySelector("[data-pf-selected]").value),null,chooser.querySelector("[data-pf-topic]").value);});game.addEventListener("click",e=>{if(e.target.closest("[data-pf-close],[data-pf-exit]"))return close();if(e.target.closest("[data-pf-ready]"))return spelling();const s=e.target.closest("[data-pf-spelling]");if(s)return resolveSpelling(s.dataset.pfSpelling,false);const m=e.target.closest("[data-pf-meaning]");if(m)return resolveMeaning(m.dataset.pfMeaning,false);if(e.target.closest("[data-pf-restart]"))start(lastCount,null,lastTopicId);});window.addEventListener("photoflash:start-request",e=>start(e.detail&&e.detail.count,e.detail&&e.detail.pool,e.detail&&e.detail.topicId));}
  ...
  window.startPhotoflash=start;window.PhotoflashBlitz=Object.freeze({version:"20260925-photoflash-topics-smart-distractors",start,close,collectPool:()=>normalizePool(),normalizePool,pickWords});
