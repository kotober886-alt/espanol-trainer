(function(){
  ...
  function results(){
    ...
    frame('<div class="pf-results"><div>📸</div><em>Спринт завершён</em><h2>'+session.total+' слов — чисто!</h2><p>'+(advanced?'Обычный круг и экзамен вслепую пройдены чисто.':'Все слова сданы без ошибок в финальной попытке.')+'</p><section><span><b>'+session.total+'</b>слов</span><span><b>'+session.retries+'</b>повторов</span><span><b>'+session.errors+'</b>ошибок / таймаутов</span><span><b>'+secs+'с</b>время</span></section><footer><button class="pf-secondary" data-pf-exit>К словам</button><button class="pf-primary" data-pf-restart>Ещё раз</button></footer></div>',"results","Готово");
  ...
  window.startPhotoflash=start;window.PhotoflashBlitz=Object.freeze({version:"20260925-photoflash-advanced-level",start,close,collectPool:()=>normalizePool(),normalizePool,pickWords});
