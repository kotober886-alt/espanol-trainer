(function(){
  ...
  function mutations(word){
    const {p,c}=splitArticle(word), out=[];const add=n=>{const v=p+n;if(n&&n!==c&&key(v)!==key(word)&&!out.includes(v))out.push(v);};const lower=c.toLowerCase();
  ...
  function meaningOptions(item){const seen=new Set([key(item.translation)]),d=[];shuffle(session.source).forEach(x=>{if(d.length>=2)return;const k=key(x.translation);if(!k||seen.has(k))return;seen.add(k);d.push(x.translation);});return shuffle([item.translation].concat(d));}
