...
class SyncManager {
  ...
  collectState() {
    return { profileCode: this.profileCode, updatedAt: Date.now(), backpack: safeParse('gato_backpack_state', {}), learnedWords: safeParse('gato_learned_words', []), errors: safeParse('gato_errors_state', []), streak: safeParse('gato_streak_state', {}), settings: safeParse('gato_settings', {}) };
  }

  queueSave() { clearTimeout(this.pending); this.pending = setTimeout(() => this.saveToCloud(), 1500); }

  installHooks() {
    window.addEventListener('backpack-item-unlocked', () => this.queueSave());
    window.addEventListener('backpack:item-unlocked', () => this.queueSave());
    window.addEventListener('gato-save-state', () => this.queueSave());
    window.addEventListener('storage', () => this.queueSave());
    document.addEventListener('visibilitychange', () => { if (document.visibilityState === 'hidden') this.queueSave(); });
    ...
    try {
      ...
      if (manager && !manager.__syncPatched && typeof manager.unlockItem === 'function') {
        ...
        try {
          manager.unlockItem = (...args) => { const result = original(...args); this.queueSave(); return result; };
          manager.__syncPatched = true;
        } catch (e) {
          console.warn('[SyncManager] Cannot patch unlockItem directly, fallback to event listener');
        }
      }
    } catch (e) {
      console.warn('[SyncManager] Cannot patch unlockItem directly, fallback to event listener');
  ...
  async saveToCloud() {
    ...
    try {
      const response = await fetch(`${FIREBASE_BASE_URL}/profiles/${encodeURIComponent(profileCode)}.json`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(this.collectState()) });
      if (response.ok) {
  ...
  async restoreFromCode(inputCode) {
    ...
    try {
      ...
      if (!response.ok) { alert('Код не найден. Проверьте правильность ввода'); return false; }
      const data = await response.json();
      if (!data) { alert('Код не найден. Проверьте правильность ввода'); return false; }
      ...
      return true;
    } catch (e) { alert('Код не найден. Проверьте правильность ввода'); return false; }
  }
}
