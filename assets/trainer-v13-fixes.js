(function(){
    ...
    if(typeof allExercises==='function'){
      ...
      allExercises=function(){
        ...
        return baseAll().filter(function(item){
          ...
          item=sanitizeContext(item,maps);
          if(item && item.type==='picture-label' && String(item.pictureScene||'')==='clothes_woman'){
            const labels=Array.isArray(item.pictureLabels)?item.pictureLabels:[];
            if(labels[0]){
              labels[0].reveal='la ropa interior blanca / el sujetador blanco';
              labels[0].answers=['ropa interior blanca','la ropa interior blanca','sujetador blanco','el sujetador blanco'];
              labels[0].translation='белое нижнее бельё / белый лифчик (бюстгальтер)';
            }
            item.displayAnswer=labels.map(function(label,index){
              const number=label&&label.displayNumber ? label.displayNumber : (index+1);
              return number+'. '+String(label&&label.reveal||'');
            }).join(' · ');
            item.a=[labels.map(function(label){return String(label&&label.reveal||'');}).join(' | ')];
          }
          return item;
        });
      };
  ...
  function loadAnswerTranslations(){
    ...
    script.src='assets/answer-translations.js?v=20260920-cloze-results1';
    ...
    script.onload=function(){
      ...
      refine.src='assets/answer-translations-refine.js?v=20260920-cloze-results1';
  ...
  try{
    if(!window.__trainerRefreshStateInstalled){
      ...
      function restoreRefreshPosition(){
        try{
          if(new URLSearchParams(window.location.search).has("resultTest")){
