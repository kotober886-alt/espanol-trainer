(function(){
  'use strict';
  const VERSION='20260917-final1';
  const SRC='./assets/generated/study-animals-v5.webp?cb='+VERSION;
  const FALLBACK='./assets/generated/study-animals-v5.webp?cb='+VERSION;
  ...
  function rasterAnimal(key){
    const p=POS[key];
    if(!p) return '';
    return '<div class="animal-raster-crop" aria-hidden="true"><img src="'+SRC+'" alt="" loading="eager" decoding="async" onerror="this.onerror=null;this.src=\''+FALLBACK+'\'" style="left:-'+(p[0]*100)+'%;top:-'+(p[1]*100)+'%;"></div>';
  }
  window.animalArt=rasterAnimal;

  const style=document.createElement('style');
  style.textContent='\n.animal-raster-crop{position:relative!important;display:block!important;width:min(280px,94%)!important;height:auto!important;aspect-ratio:1/1!important;overflow:hidden!important;margin:auto!important;border-radius:18px!important;background:#fffdf8!important}.animal-raster-crop img{position:absolute!important;display:block!important;width:900%!important;height:400%!important;max-width:none!important;max-height:none!important;object-fit:fill!important;margin:0!important;padding:0!important}.food-art .animal-raster-crop{width:min(280px,94%)!important}.visual-single-art .animal-raster-crop{width:min(230px,76%)!important}@media(max-width:520px){.food-art .animal-raster-crop{width:min(220px,94%)!important}.visual-single-art .animal-raster-crop{width:min(175px,76%)!important}}\n';
  document.head.appendChild(style);

  try{
    if(typeof studyArt==='function' && !studyArt.__animalRasterFinal){
      ...
      patchedStudyArt.__animalRasterFinal=true;
      studyArt=patchedStudyArt;
    }
  }catch(e){}

  try{
    if(typeof allExercises==='function' && !allExercises.__animalRasterFinal){
      ...
      patchedAll.__animalRasterFinal=true;
      allExercises=patchedAll;
    }
  }catch(e){}

  try{
    if(typeof renderStudy==='function' && !renderStudy.__animalRasterFinal){
      ...
      patchedRender.__animalRasterFinal=true;
