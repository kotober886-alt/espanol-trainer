(function(){
  const IMG={
    clothes:window.GEN_IMG_CLOTHES,
    foods:window.GEN_IMG_FOODS,
    activities:window.GEN_IMG_ACTIVITIES
  };
  const MAP={
  ...
  function hqItem(type,pos){
    const bucket=window.GEN_HQ_ITEMS&&window.GEN_HQ_ITEMS[type];
    return bucket&&pos?bucket[pos[0]+','+pos[1]]:'';
  }
  function itemImg(type,pos,extraClass){
    const src=hqItem(type,pos);
    if(!src) return '';
    return '<img class="generated-item-img generated-item-'+type+(extraClass?' '+extraClass:'')+'" src="'+src+'" alt="" aria-hidden="true" loading="eager" decoding="async">';
  }
  function sprite(type,key,cols,rows){
    const pos=(MAP[type]||{})[key];
    if(!pos) return '';
    const hq=itemImg(type,pos,'');
    if(hq) return hq;
    if(!IMG[type]) return '';
  ...
  const oldPicture=window.pictureScene;
  window.pictureScene=function(kind){
    if(/^clothes_/.test(kind)){
      const keys=['dress','coat','sweater','jacket','skirt','trousers','jeans','scarf','hat','sneakers'];
      const n=Math.abs(Array.from(kind).reduce(function(a,c){return a+c.charCodeAt(0);},0))%keys.length;
      const pos=MAP.clothes[keys[n]];
      const hq=itemImg('clothes',pos,'picture-scene picture-raster generated-item-picture');
      if(hq) return hq;
      return '<div class="picture-scene generated-sprite picture-raster" style="background-image:url('+IMG.clothes+');background-size:600% 500%;background-position:'+((pos[0]/5)*100)+'% '+((pos[1]/4)*100)+'%;"></div>';
    }
    if(/^food_/.test(kind)||/^dish_/.test(kind)){
      const key=/salad/.test(kind)?'salad':/cake|flan|cookies/.test(kind)?'cake':/pizza/.test(kind)?'pizza':/stew|soup/.test(kind)?'soup':'sandwich';
      const pos=MAP.foods[key];
      const hq=itemImg('foods',pos,'picture-scene picture-raster generated-item-picture');
      if(hq) return hq;
      return '<div class="picture-scene generated-sprite picture-raster" style="background-image:url('+IMG.foods+');background-size:700% 500%;background-position:'+((pos[0]/6)*100)+'% '+((pos[1]/4)*100)+'%;"></div>';
    }
    return oldPicture?oldPicture(kind):'';
  };
})();
